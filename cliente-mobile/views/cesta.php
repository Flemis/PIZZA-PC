	...
	<label>CESTA</label>
			...
			<table style="border: 1px solid black; width: 70%;margin: 0 auto;background-color: #ccc;text-align: center">  
				...
				<?php 	
						$total = 0.0;	
						$count = 1;
						while ($linha_pedidos = mysqli_fetch_array($consulta_cesta)) {
							if ($linha_pedidos['id_cliente'] == $_SESSION['id_cliente']){
								while ($linha_pizza = mysqli_fetch_array($consulta_produto_pizza)) {
									if($linha_pedidos['id_pizza'] == $linha_pizza['id_pizza']){
										...
										echo '<td>'?> <a href="deleta_produto.php?id_pizza=<?php echo $linha_pizza['id_pizza'];?>&id_poc=<?php echo $linha_pedidos['id_poc'];?>">X</a> <?php '</td></tr>';
								...
								while ($linha_bebida = mysqli_fetch_array($consulta_produto_bebida)) {
									if($linha_pedidos['id_bebida'] == $linha_bebida['id_bebida']){
										...
										echo '<td>'?> <a href="deleta_produto.php?id_bebida=<?php echo $linha_bebida['id_bebida'];?>&id_poc=<?php echo $linha_pedidos['id_poc'];?>">X</a> <?php '</td></tr>';
								...
								while ($linha_sobremesa = mysqli_fetch_array($consulta_produto_sobremesa)) {
									if($linha_pedidos['id_sobremesa'] == $linha_sobremesa['id_sobremesa']){
										...
										echo '<td>'?> <a href="deleta_produto.php?id_sobremesa=<?php echo $linha_sobremesa['id_sobremesa'];?>&id_poc=<?php echo $linha_pedidos['id_poc'];?>">X</a> <?php '</td></tr>';	
									}
								}
		...
		<a href="?pagina=fecha_cesta">
			<input type="button" name="confirmar" value="fechar compra" style="margin-bottom: 10px">
		</a>
		</div>
